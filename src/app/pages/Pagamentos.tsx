...
export default function Pagamentos() {
  ...
  async function cancelPayment(id: number) {
    ...
    try {
      ...
      alert(error.response?.data?.message ?? "Erro ao cancelar pagamento.");
    }
  }
        ...
        {(["PENDING", "APPROVED", "PAID", "CANCELLED"] as PaymentStatus[]).map(
          (status) => {
            ...
            return (
              <div
                key={status}
                className="bg-card border border-border rounded-lg p-4 sm:p-6 min-w-0"
              >
                <div
                  className={`
                ...
                <div className="flex gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                      <span>R$ {formatAmount(payment.amount)}</span>
