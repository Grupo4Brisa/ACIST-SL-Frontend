...
export default function AprovacaoCadastros() {
  ...
  async function handleAprovar(id: number) {
    const confirmar = confirm("Deseja realmente aprovar este cadastro?");
  ...
  const cadastrosFiltrados = cadastros.filter((cadastro) => {
    ...
    if (selectedFilter === "aprovado" && cadastro.status === "ACTIVE")
      return true;

    if (selectedFilter === "reprovado" && cadastro.status === "INACTIVE")
      return true;
  ...
  const aprovadosCount = cadastros.filter((c) => c.status === "ACTIVE").length;
        ...
        {cadastrosFiltrados.length === 0 ? (
          ...
          cadastrosFiltrados.map((cadastro) => {
            ...
            return (
              <div
                key={cadastro.id}
                className="bg-card rounded-lg border border-border p-4 sm:p-6 hover:border-[#5DA5FF] transition-all"
              >
                ...
                <div className="flex gap-3 flex-wrap border-t border-border pt-4">
                  <button
                    onClick={() => navigate(`/admin/company/${cadastro.id}`)}
                  ...
                  {cadastro.status === "PENDING_APPROVAL" && podeAprovar && (
                    <>
                      <button
                  ...
                  {cadastro.status === "PENDING_APPROVAL" && !podeAprovar && (
                    <div className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm">
                      Apenas usuários com perfil de aprovador podem aprovar ou
                      reprovar cadastros.
                    </div>
                  )}
                </div>
