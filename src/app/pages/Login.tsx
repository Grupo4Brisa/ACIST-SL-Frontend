...
export default function Login() {
  ...
  const [formData, setFormData] = useState({
    email: '',
    senha: '',
    perfil: 'administrador' // 'administrador' | 'aprovador'
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const success = login(formData.email, formData.senha, formData.perfil);
      setIsLoading(false);

      if (success) {
        // Redirecionar baseado no perfil
        if (formData.perfil === 'aprovador') {
        ...
        <div className="w-full max-w-6xl grid md:grid-cols-2 gap-0 bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 md:p-12">
            <div className="text-center mb-6">
              <p className="text-[#0C3A59] text-[1.125rem] font-semibold mb-2">Área do Colaborador</p>
              <h2 className="mb-2">Entrar no Sistema</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <input
                    ...
                    name="perfil"
                    value="aprovador"
                    checked={formData.perfil === 'aprovador'}
                    onChange={e => setFormData({ ...formData, perfil: e.target.value })}
                    className="sr-only"
                    disabled={isLoading}
                  />
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-[0.875rem] text-muted-foreground">Lembrar-me</span>
              </label>
              <a href="#" className="text-[0.875rem] text-primary hover:underline">
                Esqueceu a senha?
              </a>
            </div>
