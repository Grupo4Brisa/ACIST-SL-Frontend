...
export default function BoasVindasHome() {
  ...
  // Remove tudo que não for dígito. Usado tanto no onChange
  // (pra já salvar limpo no estado) quanto como segurança extra
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: Record<string, any> = {
        cnpjcpf: onlyDigits(formData.cnpjcpf),
      ...
      const response = await api.post("/companies/landing", payload);
      ...
      localStorage.setItem(
        "companyData",
        JSON.stringify({ ...formData, companyId: company.id }),
      );

      navigate(`/pagamento-pix?porte=${formData.companySize}`);
    } catch (err: any) {
      setError(err.response?.data?.message || "Erro ao iniciar cadastro");
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-blue-100 text-[1.25rem] max-w-3xl mx-auto">
            Junte-se à principal associação comercial, industrial, de serviços e
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  autoComplete="off"
                  maxLength={14}
                  placeholder="Ex: 12345678900 (CPF) ou 12345678000199 (CNPJ)"
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      cnpjcpf: onlyDigits(e.target.value),
                    })
                  }
                ...
                <p className="mt-1.5 text-[0.75rem] text-gray-500">
                  Digite apenas os números, sem pontos, traço ou barra (CPF: 11
                  dígitos · CNPJ: 14 dígitos).
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  autoComplete="off"
                  maxLength={11}
                  placeholder="Ex: 51999999999 (DDD + número)"
                  className="w-full px-4 py-3.5 border border-border rounded-lg bg-input-background focus:outline-none focus:ring-2 focus:ring-[#5DA5FF] transition-all"
                ...
                <p className="mt-1.5 text-[0.75rem] text-gray-500">
                  Digite apenas os números, com DDD, sem parênteses, espaço ou
                  traço.
                </p>
              </div>
        ...
        <div className="bg-[#F4F8FA] rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-600 mb-8 max-w-2xl mx-auto text-[1.125rem]">
            A ACIST São Leopoldo representa os interesses das empresas da região
            há mais de 50 anos. Junte-se a nós e fortaleça sua presença no
            mercado.
          </p>
